...
def podatki_o_pricakovani_zivljenjski_dobi_po_drzavah():
    ...
    for vrstica in vrstice:
        ...
        # Pričakovana življenjska doba - povprečje spolov.
        pricakovana_zivljenjska_doba = re.search(r'<td dir="ltr" class="px-2 border-e border-zinc-200 text-end py-1.5 border-b font-bold" data-order="([\s\S]*?)"> ([\s\S]*?) </td>', vrstica)

        # Pričakovana življenjska doba za ženske se pridobi iz istega vzorca.
        pricakovana_zivljenjska_doba_zenske = re.search(r'<td dir="ltr" class="px-2 border-e border-zinc-200 text-end py-1.5 border-b bg-fuchsia-100" data-order="([\s\S]*?)"> ([\s\S]*?) </td>', vrstica)

        # Pričakovana življenjska doba za moške se pridobi iz malo drugačnega vzorca, saj je razlika v barvi podatka.
        pricakovana_zivljenjska_doba_moski = re.search(r'<td dir="ltr" class="px-2 border-e border-zinc-200 text-end py-1.5 border-b bg-cyan-100" data-order="([\s\S]*?)"> ([\s\S]*?) </td>', vrstica)

        seznam_slovarjev.append({
                'Država': drzava.group(2),
                'Pričakovana življenjska doba': pricakovana_zivljenjska_doba.group(2),
                'Pričakovana življenjska doba (ženske)': pricakovana_zivljenjska_doba_zenske.group(2),
                'Pričakovana življenjska doba (moški)': pricakovana_zivljenjska_doba_moski.group(2)
            })
